  ...
  implements ICommandHandler<DeleteVariantCommand>
{
  ...
  async execute(command: DeleteVariantCommand): Promise<void> {
    ...
    const updatedProduct = this.eventPublisher.mergeObjectContext(
      Product.removeVariant(product, identifier, identifierType, attributeKey),
    );

    // Check if the variant was actually removed (product reference changed)
    if (updatedProduct === product) {
      this.logger.warn(
        `There was a problem trying to delete the variant with ${identifierType} ${identifier} not found in product ${productId}`,
      );
      throw new NotFoundException(
        `Variant with ${identifierType} ${identifier} not found in product ${productId}`,
      );
    }
